//
//  Example/Tests/MobileMessagingTests/UserDataTests.swift
...
class UserDataTests: MMTestCase {
	func testDataPersisting() async throws {
        ...
        try await MobileMessaging.saveUser(u)
        
		// Assert delta content
		let u2 = MobileMessaging.getUser()!
		XCTAssertEqual(u2.customAttributes?["nickname"] as? String, "Crusher")
		XCTAssertEqual(u2.customAttributes?["nilElement"] as? NSNull, NSNull())
		XCTAssertEqual(u2.phones, ["123"])
		XCTAssertEqual(u2.birthday, darthVaderDateOfBirth)
		XCTAssertEqual(u2.emails, ["some@mail.com"])
	}
...
"""
	...
	func testUserDataFetching() async throws {
		MMTestCase.startWithCorrectApplicationCode()
	...
	func testThatFetchedUserDataIgnoredIfHasUnsyncedLocalChanges() {
		...
		weak let expectation = self.expectation(description: "data received")
		weak let expectationAPICallPerformed = self.expectation(description: "expectationAPICallPerformed")
		mobileMessagingInstance.pushRegistrationId = MMTestConstants.kTestCorrectInternalID
	...
	func testThatUnwantedMergeErrorIsPorpagated() async throws {
		MMTestCase.startWithCorrectApplicationCode()
	...
	"""
		...
		do {
			try await MobileMessaging.saveUser(user)
	...
	func testThatAfterMergeInterrupted_UserIdentityRollsBack() async throws {
		MMTestCase.startWithCorrectApplicationCode()
		...
		await performMergeInterruptedUserUpdateCase(user: user)

		let userAfter = MobileMessaging.getUser()!
		XCTAssertNil(userAfter.phones)
		XCTAssertNil(userAfter.emails)
		XCTAssertNil(userAfter.externalUserId)
	}

	func testThatDirtyUserAttributesSentToServer() async throws {
		MMTestCase.startWithCorrectApplicationCode()
		...
		let user1 = MobileMessaging.getUser()!
		user1.firstName = "A"
		user1.customAttributes = ["string": "x" as NSString, "bool": true as NSNumber, "num": 9.5 as NSNumber, "bool2": true as NSNumber, "num2": 9.5 as NSNumber, "empty": "empty" as NSString]
		try await MobileMessaging.saveUser(user1)

		let user2 = MobileMessaging.getUser()!
		user2.firstName = "B"
		user2.customAttributes = ["string": "y" as NSString, "bool": false as NSNumber, "num": 10 as NSNumber, "bool2": true as NSNumber, "num2": 9.5 as NSNumber, "empty": NSNull()]
		try await MobileMessaging.saveUser(user2)
	...
	func testThatEmptyCustomAttributesSentAsNull() async throws {
		MMTestCase.startWithCorrectApplicationCode()
		...
		let user1 = MobileMessaging.getUser()!
		user1.customAttributes = ["string": "x" as NSString, "bool": true as NSNumber, "num": 9.5 as NSNumber, "bool2": true as NSNumber, "num2": 9.5 as NSNumber]
		try await MobileMessaging.saveUser(user1)

		let user2 = MobileMessaging.getUser()!
		user2.customAttributes = [:]
		try await MobileMessaging.saveUser(user2)
...
func performMergeInterruptedUserUpdateCase(user: MMUser) async {
	let remoteApiProvider = RemoteAPIProviderStub()
	remoteApiProvider.patchUserClosure = { (_, _, _) -> UpdateUserDataResult in
		let jsonStr = """
		{
