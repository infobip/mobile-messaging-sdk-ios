...
class EscapingQueryTests : XCTestCase {
	...
	func testNumber() {
		let query = MMHTTPRequestSerializer.queryFromParameters(parameters: ["number":12321432 as AnyObject])
		XCTAssertEqual("number=12321432", query)
	}
	
	func testEscape() {
		let query = MMHTTPRequestSerializer.queryFromParameters(parameters: ["Key!*'();:@&=+$,/?%#[]": "Value!*'();:@&=+$,/?%#[]" as AnyObject])
