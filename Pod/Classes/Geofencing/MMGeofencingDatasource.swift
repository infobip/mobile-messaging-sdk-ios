...
class MMGeofencingDatasource {
	...
	var messages = Set<MMGeoMessage>() {
		didSet {
			messages.forEach { addRegions(fromMessage: $0) }
		}
	}
...
//MARK: - Private
	...
	private func addRegions(fromMessage message: MMGeoMessage) {
	...
	private func regions(from campaigns: Set<MMGeoMessage>) -> [MMRegion] {
		return campaigns.reduce([MMRegion](), combine: { (regions, message) -> [MMRegion] in
