...
class Operation: Foundation.Operation {
	...
	*/
	...
	func cancelWithErrors(_ errors: [NSError]) {
	...
	internal final func produceOperation(_ operation: Foundation.Operation) {
		for observer in observers {
	...
	*/
	internal final func finishWithError(_ error: NSError?) {
		if let error = error {
			finish([error])
		}
		else {
