// 
//  ChatExample/MobileChatExample/ExternalChatInputViewController.swift
//  MobileChatExample
//
...
class ExternalChatInputViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    ...
    @objc func sendText() {
        if let text = externalChatInput.textField.text {
            ...
            Task {
                do {
                    try await chatView.send(text.livechatBasicPayload)
                } catch {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let image = info[.originalImage] as? UIImage {
            ...
            Task {
                do {
                    try await self.chatView.send(payload)
                } catch {
