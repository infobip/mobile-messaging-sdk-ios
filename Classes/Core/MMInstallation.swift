...
final public class MMInstallation: NSObject {
	...
	public func syncPrimarySettingWithServer(_ completion: ((NSError?) -> Void)? = nil) {
	...
	func logout(callAndForget: Bool, completion: @escaping (NSError?) -> Void) {
		_ = installationQueue.addOperation(LogoutOperation(mmContext: mmContext, callAndForget: callAndForget, finishBlock: completion))
	}
	...
	func setAsPrimaryDevice(_ isPrimary: Bool, completion: ((NSError?) -> Void)? = nil) {
		...
		syncPrimarySettingWithServer(completion)
	}
