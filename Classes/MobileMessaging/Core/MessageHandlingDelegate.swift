...
@objc public protocol MMMessageHandlingDelegate {
    ...
    /// Called when an in-app notification with action "open live chat" is tapped. If not implemented, in-app chat will be automatically presented, with default input composer and in the top view controller. Implement this method if you wish to have full control on how the chat will be presented and what input composer will be used (as you can define your own).
    /// - parameter message: `MM_MTMessage` object
    @objc optional func inAppOpenLivechatNotificationTapped(for message: MM_MTMessage)
}
