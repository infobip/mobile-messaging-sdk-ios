...
extension UIApplication: MMApplication {
    public var visibleViewController: UIViewController? {
        ...
        return self.firstKeyWindow?.visibleViewController
    }
}
...
class MainThreadedUIApplication: MMApplication {
    ...
    var visibleViewController: UIViewController? {
        return getFromMain(getter: { app.firstKeyWindow?.visibleViewController })
    }
...
extension UIColor {
    ...
    public convenience init(hexString: String, alpha: CGFloat = 1.0) {
        let hexString: String = hexString.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines).replacingOccurrences(of: "#", with: "")
        let scanner = Scanner(string: hexString)
        let color = scanner.scanInt(representation: .hexadecimal) ?? 0
        let mask = 0x000000FF
