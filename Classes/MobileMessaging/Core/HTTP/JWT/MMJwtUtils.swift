...
public let MMJwtValidationErrorDetails = "com.mobile-messaging.jwtValidationErrorDetails"
...
final class MMJwtValidator {
    ...
    static func validateStructure(_ jwt: String) throws {
        if jwt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            throw NSError(type: .JwtStructureValidationError, detail: [MMJwtValidationErrorDetails: "Token is empty or blank."])
        }
        let parts = jwt.split(separator: ".")
        if parts.count != 3 {
            throw NSError(type: .JwtStructureValidationError, detail: [MMJwtValidationErrorDetails: "Token must have three parts separated by dots."])
        }
        // Header
        guard
            ...
            throw NSError(type: .JwtStructureValidationError, detail: [MMJwtValidationErrorDetails: "Token header is not a valid Base64 encoded JSON object."])
        }
        ...
        if !mandatoryHeaders.isSubset(of: headerKeys) {
            ...
            throw NSError(
                type: .JwtStructureValidationError,
                detail: [MMJwtValidationErrorDetails: "Missing JWT header fields: \(Array(missing))"]
            )
        }
        // Payload
        guard
            ...
            throw NSError(type: .JwtStructureValidationError, detail: [MMJwtValidationErrorDetails: "Token payload is not a valid Base64 encoded JSON object."])
        }
        ...
        if !mandatoryClaims.isSubset(of: claimKeys) {
            ...
            throw NSError(
                type: .JwtStructureValidationError,
                detail: [MMJwtValidationErrorDetails: "Missing JWT claims: \(Array(missing))"]
            )
        }
    }
