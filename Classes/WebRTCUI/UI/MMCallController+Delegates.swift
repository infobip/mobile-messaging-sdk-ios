...
extension MMCallController {
    fileprivate func ringing(callType: MMCallType) {
        self.startRingback()
    ...
    fileprivate func hangup(errorCodeName: String) {
        self.finalizeCallPreview("Status: \(errorCodeName)")
        self.stopRingback()
    }
}

extension MMCallController: CallDelegate {
    ...
    public func onUpdated(_ callUpdatedEvent: CallUpdatedEvent) {
        DispatchQueue.main.async { [weak self] in
            self?.updated(remoteVideoTrack: callUpdatedEvent.remoteVideoTrack)
            self?.updated(localVideoTrack: callUpdatedEvent.localVideoTrack)
        }
    }
    ...
    public func onError(_ callErrorEvent: CallErrorEvent) {
        if let localisedErrorMsg = callErrorEvent.reason.id.mmLocalisedCallErrorEvent {
            // We apply our interpretation of any weird code coming from webrtc
            self.showErrorAlert(message: localisedErrorMsg)
        }
        self.finalizeCallPreview("Something unexpected happened.")
        self.stopRingback()
    }
