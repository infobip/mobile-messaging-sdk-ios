...
open class MMMessageComposingViewController: MMKeyboardAwareScrollViewController, MMComposeBarDelegate, UIGestureRecognizerDelegate,
    ...
    public func sendDraft(_ message: String?, completion: @escaping (_ error: NSError?) -> Void) { /* override */ }
    public func textDidChange(_ text: String?, completion: @escaping (_ error: NSError?) -> Void) { /* override */ }
    public func send(_ payload: MMLivechatPayload, completion: @escaping (_ error: NSError?) -> Void) { /* override */ }
    ...
    internal func brandComposer() {
        ...
        composeBarView.charCounterLabel.font = MMChatSettings.getCharCounterFont()
        composeBarView.backgroundView.barTintColor = settings?.backgroundColor ?? UIColor(white: 1, alpha: 1)
        composeBarView.backgroundView.tintColor = settings?.backgroundColor ?? UIColor(white: 1, alpha: 1)
        ...
        if MMLanguage.sessionLanguage.isRightToLeft { composeBarView.adjustLayoutForRTL() }
    }
