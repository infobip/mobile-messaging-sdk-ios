...
class WebInAppBannerManager: NSObject, NamedLogger, WKNavigationDelegate, UserInteractionEventListener {
    ...
    private func initializeDynamicConstraints() {
        msgViewHeight = msgView.heightAnchor.constraint(equalToConstant: WebInAppBannerManager.initialBannerHeight)
    ...
    /// Updates the height immediatelly or schedules a later update in case the page is not loaded yet.
    private func handleMsgViewHeight() {
        switch mode {
        case .preloadedWebViewWithHeightProvided(let height):
            // Update the height immediatelly because it's already known in advance.
            msgViewHeight.constant = height
        case .fromScratch:
            // Schedule height update for later once the page is loaded.
